...
export const SessionSchema = z
  .object({
    ...
    venue: z
      .object({
        name: z.string().min(1).max(100, { message: "Venue name cannot exceed 100 characters" }),
        locality: z
          .string()
          .min(1)
          .max(300, { message: "Venue address cannot exceed 300 characters" }),
        city: z.string().min(1).max(100, { message: "City cannot exceed 100 characters" }),
        state: z.string().min(1).max(100, { message: "State cannot exceed 100 characters" }),
        country: z.string().min(1).max(100, { message: "Country cannot exceed 100 characters" }),
        zipCode: z.string().min(1).max(20, { message: "Zip code cannot exceed 20 characters" }),
      })
      .optional(),
...
export const TicketSchema = z
  .object({
    ...
    salesStartTime: z.coerce.date({ message: "Invalid sales start time" }).optional(),
    salesEndTime: z.coerce.date({ message: "Invalid sales end time" }).optional(),
...
export const EventBaseSchema = z
  .object({
    ...
    registrationStart: z.coerce.date({ message: "Invalid registration start date" }),
    registrationEnd: z.coerce.date({ message: "Invalid registration end date" }),
...
export const EventSchema = EventBaseSchema.safeExtend({
  ...
  coupons: z
    ...
    .default([]),
  metrics: EventMatricesSchema.default({ view: 0, paidRegistrations: 0, freeRegistrations: 0 }),

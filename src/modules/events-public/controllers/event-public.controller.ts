...
export class EventPublicController {
  static listPublicEvents = asyncHandler(
    async (req: Request, res: Response) => {
      ...
      const result = await eventPublicRepository.listPublicEvents(
        filters,
        limit,
        sort,
        page,
  ...
  static getPublicEvent = asyncHandler(async (req: Request, res: Response) => {
    ...
    const filters = buildPublicEventLookup(
      eventId,
      req.query.includeUnlisted as string | undefined,
    );
    ...
    res.status(200).json({
      success: true,
      data: serializePublicEvent(event),
    });
  });
}

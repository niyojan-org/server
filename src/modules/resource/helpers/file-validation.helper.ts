...
const isAllowedMimeType = <T extends readonly string[]>(list: T, value: string): value is T[number] =>
...
export function validateMimeType(mimetype: string): void {
  ...
  if (!isAllowedMimeType(allAllowedTypes, mimetype)) {
    throw new ApiError(400, 'Unsupported file type', 'UNSUPPORTED_FILE_TYPE', `File type ${mimetype} is not allowed`);
  }
}

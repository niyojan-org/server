...
export const getOrganizationAdmin = asyncHandler(
  async (req: OrganizationRequest, res) => {
    const organization = getOrganizationView(
      req.organization,
...
export const raiseOrganizationVerification = asyncHandler(
  async (req: OrganizationRequest, res) => {
    await raiseOrgVerification(req);
    res.status(200).json({
      success: true,
      message: 'Organization verification request raised successfully',
    });
  },
);
